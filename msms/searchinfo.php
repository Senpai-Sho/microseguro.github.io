...
$sql = "SELECT * FROM member_info WHERE account_number LIKE '$search%' || full_name LIKE '%$search%' ORDER BY full_name  ";
$memberinfo = $con->query($sql) or die ($con->connect_error);
$row = $memberinfo->fetch_assoc();

$total = $memberinfo->num_rows;
...
.dropdown:hover .dropdown-content {
  ...
  <div class="content">
    ...
    <div class="container">
      <br>
      <br>
      <h2 class="mb-5">Member Information </h2>

      <form action ="searchinfo.php" method="get">
        <table>

        <?php if($_SESSION['Access'] == "Guest" || $_SESSION['Access'] ==  "MS-Master"){?>
            
            <?php echo "";?>

              <?php }else{?>
                <td><a href="memberselect.php"><i class="fa fa-file-text-o" style="font-size:30px;color:green"></i></a>  &nbsp;  &nbsp; </td>
        ...
        <table class="table custom-table">
          ...
          <tbody>
            ...
            <tr scope="row"> 
              ...
              <?php if($row['product_type'] == "CBK-Member"){?>
              <td><a href="memberproduct.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:green"></i></a>   </td>
              <?php }?>

              <?php if($row['product_type'] == "CBK-Pension"){?>
              <td><a href="memberproduct.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:green"></i></a>   </td>
              <?php }?>

              <?php if($row['product_type'] == "ASB-Senior"){?>
              <td><a href="memberproduct.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:green"></i></a>   </td>
              <?php }?>

              <?php if($row['product_type'] == "ASB-Individual"){?>
              <td><a href="memberproduct.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:green"></i></a>   </td>
              <?php }?>

              <?php if($row['product_type'] == "ASB-Family"){?>
              <td><a href="memberproduct.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:green"></i></a>   </td>
              <?php }?>


              <?php if($row['product_type'] == "CGL-Loan Insurance"){?>
                <td><a href="cglinfo.php?AccountNumber=<?php echo $row['account_number'];?>"><i class="fa fa-file-text-o" style="font-size:24px;color:yellow"></i></a>   </td>
              <?php }?> 
