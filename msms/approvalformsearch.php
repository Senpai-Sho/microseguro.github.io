...
$sql = "SELECT * FROM formnumber_data WHERE account_number LIKE '$search%' || full_name LIKE '%$search%' || form_number LIKE '$search%' ORDER BY account_number DESC";
$approvalform = $con->query($sql) or die (con->connect_error);
$row = $approvalform->fetch_assoc();

$total = $approvalform->num_rows;

if($total>0) {
}
else { 
  echo header("Location: approvalform.php");
}
