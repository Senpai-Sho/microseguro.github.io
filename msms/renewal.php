...
h3,h1 ,h4{
  ...
  <form action="exportrenewal.php" method="POST">
...
<td><input   type="hidden" name="branch" value = "<?php if(isset($_GET['branchname'])) { echo $_GET['branchname'];}?>" class="form-control"></td>
  ...
  <li style="float:right"><a><button onclick="exportchecker()" type="submit" class="import" name="submit"><i class="fa fa-file-excel-o" style="font-size:23px;color:green"></i> <b> EXPORT </b></button></li></a>
...
<H3>MICRO SEGURO  CLIENT RENEWAL REPORT </H3></th>
...
<br>
      ...
      if(isset($_GET['from_date']) && isset($_GET['to_date']) && isset($_GET['branchname']) ){
...
<?php
    ...
    foreach($query_run as $row){
        ...
        <?php if($row['Status'] == "Closed" || $row['Status'] == "Renewed" || $row['product_type']== "CGL-Loan Insurance"){

          echo "<td></td> ";

        }else if($_SESSION['Access'] == "MS Supervisor"|| $_SESSION['Access'] == "Developer"){?>
                
                <form action="clientrenewal.php" method="get">
          <input  type="hidden" name="from"  value="<?php if(isset($_GET['from_date'])) { echo $_GET['from_date'];}?>" class="form-control">
          <input   type="hidden" name="to" value = "<?php if(isset($_GET['to_date'])) { echo $_GET['to_date'];}?>" class="form-control">
          <input   type="hidden" name="branch" value = "<?php if(isset($_GET['branchname'])) { echo $_GET['branchname'];}?>" class="form-control">

         <td><button type="submit" id="submit-btn" class="button button1" name="submits"><i class="fa fa-user-plus" style="font-size:25px;color:green"></i></button>
         <input type="hidden"  name="AccountNumber" value = "<?php echo $row['account_number'];?>"> </td>
              </form>
          ...
          <input   type="hidden" name="branch" value = "<?php if(isset($_GET['branchname'])) { echo $_GET['branchname'];}?>" class="form-control">
                ...
                <td><button onclick="checker()" type="submit" id="submit-btn" class="button1" name="close"><i class="fa fa-user-times" style="font-size:25px;color:red"></i></button>
                <input type="hidden"  name="AccountNumber" value = "<?php echo $row['account_number'];?>"> </td>
              </form>
...
function checker(){
  ...
  var result = confirm('ARE YOU SURE YOU WANT TO CLOSED THIS ACCOUNT ?');
...
function exportchecker(){

  var result = confirm('DO YOU WANT TO EXPORT THIS BORDEREAUX');
