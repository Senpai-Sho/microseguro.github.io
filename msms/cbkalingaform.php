...
if(isset($_POST['submit'])){
  ...
  $membershipinfos = $_POST['membershipinfo'];
  ...
  $cb = "SELECT cb_number FROM cb_form WHERE cb_number = '$nocb' ";
  $ncb = mysqli_query($con,$cb);
...
if(empty($nocb)){
  $errors['cb'] = "No. CB Required";
}else if (mysqli_num_rows($ncb)>0){
  $errors['cb'] = "No. CB exist";
}
  ...
  VALUES ('$account','$fname','$civil','$memdate','$bday','$gender','$adds','$product','$nocb','$membershipinfos','$insured','$renewal','$branch',
...
$sql4="SELECT * FROM membership_info ORDER BY membership_id";
$membershipinfo = $con->query($sql4) or die($con->connect_error);
$row4 = $membershipinfo->fetch_assoc();
...
<h2>CB KALINGA MEMBER INSURANCE APPLICATION ( MEMBER ) </h2>
...
<table>
  ...
  <tr>
    ...
    <td> <input type="text" value="<?php echo $row['account_number'];?>"   id="accountnumber" name="accountnumber" autocomplete="off" READONLY  > <a href="cbkalinga.php"><i class="fa fa-file-text-o" style="font-size:20px;color:green"></i></a></td>
    <td>Member Name:</td>
...
<tr>
<td>No. CB:</td>
<td><input type="number" id="nocb" name="nocb" autocomplete="off">
<p style="color:red;"><?php if (isset($errors['cb'])) echo $errors['cb']; ?></p></td>

<td> Membership Status:</td>
<td><select id="id" name="membershipinfo">
  <?php do{?>
    <option value="<?php echo $row4['membership_name']?>"><?php echo $row4['membership_name']?></option>
   
  <?php } while ($row4= $membershipinfo->fetch_assoc())?>
  </select>
  </td>
</tr>
...
<tr>
  ...
  <td>Child2 Middlename:</td>
  <td><input type="text" id="a4middlename" name="a4middlename" autocomplete="off"></td>
  <td>child2 Lastname:</td>
