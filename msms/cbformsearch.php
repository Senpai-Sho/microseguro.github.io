...
$sql = "SELECT * FROM cb_form WHERE account_number LIKE '$search%' || full_name LIKE '%$search%' || cb_number LIKE '$search%' ORDER BY account_number DESC";
$cbform = $con->query($sql) or die (con->connect_error);
$row = $cbform->fetch_assoc();

$total = $cbform->num_rows;

if($total>0) {
}
else { 
  echo header("Location: cbform.php");
}
...
<H3>MICRO SEGURO CB-KALINGA FORM NUMBER </H3></th>
      ...
      <form action ="cbformsearch.php" method="get">
        <table>
