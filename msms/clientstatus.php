...
$sql = "SELECT * FROM client_status ORDER BY id ";
$clientstatus = $con->query($sql) or die ($con->connect_error);
$row = $clientstatus->fetch_assoc();
...
h3,h1 ,h4{
  ...
  <form action="exploreclientstatus.php" method="POST">
...
<td><input   type="hidden" name="client" value = "<?php if(isset($_GET['clientstatus'])) { echo $_GET['clientstatus'];}?>" class="form-control"></td>
...
<H3>MICRO SEGURO CLIENT STATUS REPORT </H3></th>
...
<td><select id= "id" name="clientstatus" class="form-control">
<?php if ($_GET['clientstatus'] == ""){
echo "";
}else{?>
<option value = "<?php if(isset($_GET['clientstatus'])){ echo $_GET['clientstatus'];}?>"><?php  echo $_GET['clientstatus'];}?>

	<?php do {?>

	<option value="<?php echo $row['status']?>"><?php echo $row['status']?></option>
	<?php } while ($row = $clientstatus->fetch_assoc())?>
...
<form action="" method="GET">
      ...
      <?php if(isset($_GET['from_date']) && isset($_GET['to_date']) && isset($_GET['clientstatus']) && isset($_GET['branchname']) ){

$from_date = $_GET['from_date'];
$to_date = $_GET['to_date'];
$status = $_GET['clientstatus'];
$branchname = $_GET['branchname'];


$query = "SELECT * FROM member_info WHERE date_insured BETWEEN '$from_date' AND '$to_date' AND Status = '$status' AND branch_name = '$branchname'";
